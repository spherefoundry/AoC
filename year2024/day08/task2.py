    ...
    for key in nodes.keys():
        ...
        for i in range(len(nodes_of_key) - 1):
            for j in range(i + 1, len(nodes_of_key)):
                ...
                candidate = pos_i
                while guard(candidate):
                    antinodes.add(candidate)
                    candidate = candidate.add(diff)

                candidate = pos_j
                while guard(candidate):
                    antinodes.add(candidate)
                    candidate = candidate.sub(diff)
...
class TestCases(unittest.TestCase):
    ...
    def test_example(self):
        self.assertEqual(task(data_example_filename()), 34)

    def test_task(self):
        self.assertEqual(task(data_filename()), 1359)
